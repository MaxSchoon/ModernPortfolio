...
def get_exchange_suffix(country_code: str) -> str | None:
    """
...
def validate_prices(price_data: pd.Series, min_points: int = 100) -> dict[str, Any]:
    ...
    """
    if price_data is None or price_data.empty:
        return {"valid": False, "error": "No data available", "nan_pct": 100}
...
def validate_prices_df(df: pd.DataFrame, ticker: str = None) -> dict[str, Any]:
    ...
    if not df.index.empty:
        ...
        result["info"].append(
            f"Date range: {start_date.date()} to {end_date.date()} ({date_range} days)"
        )
