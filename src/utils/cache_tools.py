...
def validate_cached_ticker(
    ticker: str, cache_dir: str = DEFAULT_CACHE_DIR, verbose: bool = True
) -> bool:
    """
    ...
    try:
        ...
        if verbose:
            print_success(
                f"{ticker}: Valid with {len(price_data)} data points over {date_range} days"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "command", choices=["list", "inspect", "fix", "validate"], help="Command to execute"
    )
