...
class DataFetcher:
    ...
    def __init__(
        ...
        throttle_period: int = 20,
    ):  # Reduced from 60 to 20 seconds
        """
    ...
    def _throttle_api_calls(self):
        ...
        self.api_call_times = [
            t for t in self.api_call_times if current_time - t < self.throttle_period
        ]
        ...
        if use_cache:
            ...
            if cached_data is not None and len(cached_data) > 100:
                ...
                if nan_pct > 50:  # If more than 50% is NaN, consider it bad data
                    ...
                    return (
                        True,
                        f"✅ {ticker}: Using cached data ({len(cached_data)} points, {nan_pct:.1f}% NaN)",
                    )
        ...
        for attempt in range(self.retry_count):
            try:
                ...
                try:
                    ...
                    if not dividends.empty:
                        ...
                        if common_dates.size > 0:
                            div_series.loc[common_dates] = dividends[
                                dividends.index.isin(common_dates)
                            ].astype("float64")
                        ...
                        return (
                            True,
                            f"✅ {ticker}: Fetched {len(prices)} price points and {len(dividends)} dividends",
                        )
                    else:
                        div_series = pd.Series(0.0, index=prices.index, dtype="float64")
                        self.cache.save_div_data(ticker, div_series)
                        ...
                        return (
                            True,
                            f"✅ {ticker}: Fetched {len(prices)} price points (no dividends)",
                        )
                    ...
                    return (
                        True,
                        f"✅ {ticker}: Fetched {len(prices)} price points (dividend error: {str(e)})",
                    )
                ...
                if attempt < self.retry_count - 1:
                    ...
                    return (
                        False,
                        f"❌ {ticker}: Failed after {self.retry_count} attempts - {str(e)}",
                    )
    ...
    def fetch_all(self, tickers: list[str], use_cache: bool = True) -> dict[str, str]:
        ...
        print(
            f"{Fore.CYAN}Fetching {len(tickers)} tickers in batches of {self.batch_size}{Style.RESET_ALL}"
        )
        print(
            f"Cache: {'Enabled' if use_cache else 'Disabled'}, Workers: {self.max_workers}, Years: {self.years}"
        )
        ...
        with tqdm(
            ...
            for i in range(0, len(tickers), self.batch_size):
                ...
                print(
                    f"Batch {batch_num} complete: {success_count} succeeded, {fail_count} failed, took {batch_time:.1f}s"
                )

                # Add delay between batches (except after the last one)
                if batch_num < batch_count:
                    self._animated_sleep(
                        self.delay_between_batches, f"Waiting before batch {batch_num + 1}"
                    )
    ...
    def process_csv_file(
        self, filepath: str, output_filepath: str = None, use_cache: bool = True
    ) -> None:
        """
...
def main():
    ...
    parser.add_argument(
        "--output", "-o", help="Output file for processed tickers", default="tickers_corrected.csv"
    )
    parser.add_argument(
        "--tickers", "-t", help="Comma-separated list of tickers to fetch", type=str
    )
    parser.add_argument(
        "--years", "-y", type=int, default=5, help="Years of historical data to fetch"
    )
    parser.add_argument(
        "--batch-size", "-b", type=int, default=10, help="Batch size for fetching"
    )  # Increased default
    parser.add_argument("--workers", "-w", type=int, default=3, help="Number of worker threads")
    parser.add_argument(
        "--delay", "-d", type=float, default=5.0, help="Delay between batches in seconds"
    )  # Fixed delay
    ...
    print(
        f"- Price data: {cache_status.get('price_ticker_count', 0)} tickers ({cache_status.get('price_cache_size_mb', 0):.2f} MB)"
    )
    print(
        f"- Date range: {cache_status.get('oldest_data', 'N/A')} to {cache_status.get('newest_data', 'N/A')}"
    )
