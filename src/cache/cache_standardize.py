...
def scan_cache_directory(cache_dir: str) -> dict[str, list[str]]:
    ...
    return {"price_files": price_files, "div_files": div_files}
...
def analyze_date_formats(files: list[str]) -> dict[str, Any]:
    ...
    for i, file in enumerate(files):
        ...
        try:
            ...
            # Inspect the first column (usually the date index)
...
def standardize_cache(cache_dir: str, remove_future: bool = True) -> dict[str, Any]:
    ...
    if stats["price_errors"] > 0 or stats["div_errors"] > 0:
        print_warning(
            f"Errors encountered: {stats['price_errors'] + stats['div_errors']} files could not be standardized"
        )
...
def main():
    ...
    parser.add_argument(
        "--verify", action="store_true", help="Verify cache alignment without making changes"
    )
    parser.add_argument(
        "--keep-future", action="store_true", help="Keep future dates (default is to remove them)"
    )
    parser.add_argument(
        "--sample", type=int, default=5, help="Number of files to sample during verification"
    )
