...
def list_cached_tickers(cache_dir: str = DEFAULT_CACHE_DIR) -> None:
    ...
    print(
        f"- Price data: {status.get('price_ticker_count', 0)} tickers ({status.get('price_cache_size_mb', 0):.2f} MB)"
    )
    print(
        f"- Dividend data: {status.get('div_ticker_count', 0)} tickers ({status.get('div_cache_size_mb', 0):.2f} MB)"
    )
...
def plot_ticker(ticker: str, cache_dir: str = DEFAULT_CACHE_DIR) -> None:
    ...
    try:
        ...
        plt.annotate(
            f"${start_price:.2f}",
            xy=(price_data.index[0], start_price),
            xytext=(10, 10),
            textcoords="offset points",
        )
        plt.annotate(
            f"${end_price:.2f}",
            xy=(price_data.index[-1], end_price),
            xytext=(-40, 10),
            textcoords="offset points",
        )
...
def test_and_repair_cached_data(
    cache_dir: str = DEFAULT_CACHE_DIR, verbose: bool = False, repair: bool = False
) -> None:
    ...
    print(
        f"Date range: {status.get('oldest_data', 'Unknown')} to {status.get('newest_data', 'Unknown')}"
    )
        ...
        if verbose:
            ...
            if not prices.empty:
                ...
                if not valid_prices.empty:
                    print(
                        f"  Date range: {valid_prices.index[0].date()} to {valid_prices.index[-1].date()}"
                    )
                ...
                if nan_pct > 50:  # Severe issues - refetch
                    try:
                        ...
                        if len(new_prices) > 10:  # We got some data
                            ...
                            print_success(
                                f"Successfully repaired {ticker} with {len(new_prices)} data points"
                            )
                        else:
...
def convert_pickle_to_csv(old_cache_dir: str, new_cache_dir: str = DEFAULT_CACHE_DIR) -> None:
    ...
    try:
        ...
        if converted_count > 0:
            print_success(
                f"Successfully converted {converted_count} out of {len(price_files)} tickers"
            )
            ...
            print(
                f"- Date range: {status.get('oldest_data', 'N/A')} to {status.get('newest_data', 'N/A')}"
            )
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Financial Data Cache Maintenance Tool")
    ...
    repair_parser.add_argument(
        "--batch", "-b", action="store_true", help="Process all cached files"
    )
    repair_parser.add_argument(
    ...
    convert_parser.add_argument(
        "--new-cache", default=DEFAULT_CACHE_DIR, help="New CSV cache directory"
    )
    ...
    if args.command == "list":
        ...
        if (
            args.confirm
            or input("Are you sure you want to clear the cache? (y/n): ").lower() == "y"
        ):
