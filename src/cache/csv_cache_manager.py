...
class CSVDataCache:
    ...
    def get_price_data(self, ticker: str, max_age_days: int = 7) -> pd.Series | None:
        """
    ...
    def get_div_data(self, ticker: str, max_age_days: int = 7) -> pd.Series | None:
        """
            ...
            try:
                ...
                if last_date < today - timedelta(days=2):  # If data is more than 2 days old
                    ...
                    extended_data = self._generate_synthetic_data(
                        ticker, start_date=last_date + timedelta(days=1), end_date=today
                    )
    ...
    def get_cache_status(self) -> dict[str, Any]:
        ...
        price_size = sum(os.path.getsize(f) for f in price_files if os.path.exists(f)) / (
            1024 * 1024
        )
...
if __name__ == "__main__":
    ...
    print("Cache Status:")
    print(
        f"- Price data: {status['price_ticker_count']} tickers ({status['price_cache_size_mb']:.2f} MB)"
    )
    print(
        f"- Dividend data: {status['div_ticker_count']} tickers ({status['div_cache_size_mb']:.2f} MB)"
    )
